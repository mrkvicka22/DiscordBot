...
async def train():
	...
	for generation in range(0,generations_planned):
		...
		for pl in range(0, len(used_lib_ai)):
			lib_players.append(used_lib_ai[random.choice(rulette)])
		...
		addition = min([player.fitness for player in fas_players])
		if addition >= 1:
			addition = 0
		else:
			addition = addition *-1 +1
		for z, plai in enumerate(fas_players):
			for fit in range(0, plai.fitness+addition):
				rulette.append(z)
		fas_players = []
		for pl in range(0, len(used_fas_ai)):
			fas_players.append(used_fas_ai[random.choice(rulette)])
			dice = random.randrange(0, randmax)/randmax
			if mutation_chance < dice:
				fas_players[-1].mutate(mutation_amplificator)
