...
class AI_player:
    ...
    def __init__(self, party):
        ...
        self.fitness = 0
        self.fraction = np.array([party])
        #davam ku kazdemu +1. Tam budeme davat ^ten party membership
        self.networks = {
            "vote":
                tf.keras.Sequential([
                    tf.keras.layers.Flatten(len(self.personal_inputs) + len(public) +1, activation=tf.nn.tanh),
                    tf.keras.layers.Dense(len(self.personal_inputs) + len(public), activation=tf.nn.tanh),
            ...
            "passing laws":
                tf.keras.Sequential([
                    tf.keras.layers.Flatten(3 + len(public)+1, activation=tf.nn.tanh),
...
async def voting(players, president, chancellor):
    ...
    for i, p in enumerate(players):
        print(p, 'Do you agree with goverment where the president would be ', president, 'and the chancellor would be',
              chancellor)
