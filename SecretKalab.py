...
async def play(gamers):
	...
	while(not end):
		...
		try: await set_public(4, players.index(chancellor))
...
async def train():
	...
	for generation in range(0, generation_size_coef * 3 - l):
		lib_players.append(AI_player("liberal", "l" + ''.join(random.choice(string.ascii_lowercase) for i in range(7))))
	l = len(fas_players)
	for generation in range(0, generation_size_coef * 2 - l):
		fas_players.append(AI_player("fascist", "f" + ''.join(random.choice(string.ascii_lowercase) for i in range(7))))
...
class AI_player:
    ...
    def mutate(self, amp):
        ...
        print(weights)
        for x in range(0, random.randrange(0, len(weights) * amp)):
            weights[random.randrange(0, len(weights))] = weights[random.randrange(0, len(weights))] - 0.001 + 0.001 * (
                        random.randrange(0, 2000) / 1000)
...
async def play(gamers):
    ...
    while (not end):
        ...
        try:
            set_public(4, players.index(chancellor))
        except ValueError:
        ...
        turnhis.append(chancellor)
        set_public(1, players.index(president))
        set_public(2, players.index(chancellor))
        if (await check(lib, fas, chancellor, roles, players, hitler)):
            end = True
            break
        if (chaos):
...
async def choseChancellor(players, president, lastPresident, lastChancellor):
    ...
    global illegal_action_punishment
    print(president, ':Who do you want as a chancellor in your government?')
    print(ai_players, 'ai players')
    print(players.index(president))
    await send(president, 'Who do you want as a chancellor in your government?')
    while (True):
        #	   if commands.empty():
        #		  continue
...
async def train():
    ...
    try:
        ...
        raise FileNotFoundError("no liberal files found")
    try:
        with open('fascist_ai', 'rb') as input:
            fas_players = pickle.load(input)
    except FileNotFoundError:
        raise FileNotFoundError("no fascist files found")
